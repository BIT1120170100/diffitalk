...
char *get_user_ip(char *username)
{
	...
	for (i = 0; i < MAX_CLIENT_COUNT; i++)
	{
		if (strcmp(username, client_prop[i].username) == 0)
		{
			return sock_ntop((sockaddr *)&client_prop[i].addr);
		}
	}
}
/**************************************************/
...
/***************************************************/
//===================SERVER LOGIC=================================
void user_register(struct client_property *prop, char *username, char *password, char *email)
{
	...
	{
		sprintf(sqlcmd, "{\"type\":\"register-receipt\", \"userid\":\"%s\", \"status\":0}", NULL);
	}
...
/*作者：李妙宇
/***************************************************/
void user_login(struct client_property *prop, char *userid, char *password)
{
	int status = 0;
	printf("login %s %s\n", userid, password);
	char sqlcmd[2048];
	memset(sqlcmd, 0, sizeof(sqlcmd));
	sprintf(sqlcmd, "select password from alluser where userid=\'%s\'", userid);
	printf(sqlcmd);
	char jsonstr[1024];

	// int status;
	int result = passWordMatch(userid, password);
	// int result = 1;
	if (result == 0)
	{
		printf("no such user\n");
	...
	else
	{
		...
		status = 1;
		//struct client_property *prop1 = get_user_prop(userid);
		//if(prop1 != NULL) {
...
{
	if (DEBUG)
		printf("send_single_message_callback %s\n", values[0]);
...
{
	if (DEBUG)
		printf("handling message from client %s, message is\n%s\n",
	...
	if (strcmp(type, "register-message") == 0)
	{
		...
		user_register(prop, username, password, email);
	}
...
void *send_thread_function(void *arg)
{
	...
	while (1)
	{
		...
		if (send_q_head != send_q_tail)
		{
			...
			{
				if (DEBUG)
					printf("sending message to online user %s: %s\n", msg.username, msg.message);
			...
			{
				if (DEBUG)
					printf("saving offline message for user %s, message is %s\n", msg.username, msg.message);
